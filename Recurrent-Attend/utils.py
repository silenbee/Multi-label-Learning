...
def loss_function(input, target, add_constraint=False):
    '''
...
class AveragePrecisionMeter(object):
    ...
    def evaluation(self, scores_, targets_):
        ...
        for k in range(n_class):
            ...
            Np[k] = np.sum(scores >= 0.8)
            Nc[k] = np.sum(targets * (scores >= 0.8))
        Np[Np == 0] = 1
        Ng[Ng == 0] = 1
        # delete 0 dim for padding
        Nc = Nc[1:]
        Np = Np[1:]
        Ng = Ng[1:]
        print("Nc: ", Nc)
        ...
        CP = np.sum(Nc / Np) / (n_class-1)  # -1 for dim 0
        CR = np.sum(Nc / Ng) / (n_class-1)
        CF1 = (2 * CP * CR) / (CP + CR)
