...
#   trianing 
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(args.epoch):
            ...
            predicts = tf.reshape(predicts, [-1, 24])
            targets = tf.reshape(caption_number, [-1])
            ...
            loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=predicts,labels=targets)
